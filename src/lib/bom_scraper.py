...
def get_first_search_data(titleName):
    ...
    movieData = pd.DataFrame(columns = ['title', 'worldwide_lifetime_gross','budget'])

    # thresholds to stop the search if there are no results
    counter = 0
    failed = 0
    for url in complete_urls:
        if counter != 1 and failed < 15:
            soup = __fetch_htmldata(url)
            try:
                title, wlg, budget = __fetch_movie_search_data(soup)
                movieData = movieData.append({'title' : title, 'worldwide_lifetime_gross' : wlg, 'budget' : budget} , ignore_index=True)
                counter += 1
            except:
                failed += 1
                movieData = movieData.append({'title' : np.nan, 'worldwide_lifetime_gross' : np.nan, 'budget' : np.nan} , ignore_index=True)
    ...
    for url in complete_urls:
        ...
        try:
            ...
            movieData = movieData.append({
                            'title' : np.nan, 'worldwide_lifetime_gross' : np.nan, 'budget' : np.nan} , ignore_index=True)
        counter += 1
...
def __fetch_movie_search_data(soup):
    ...
    return title, worldwide_lifetime_gross, budget
